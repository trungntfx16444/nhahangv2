    ...
    public class CommonFunc
    {
      ...
      return false;
        ...
        internal static string GetCardTypeByCardNumber(string cardNumber)
        {
            ...
            // visa 
            var checkVisa = new Regex("^(?:4[0-9]{12}(?:[0-9]{3})?)$");
            ...
            // amex
            var checkAmex = new Regex("^(3[47][0-9]{13})$");
            ...
            // discover
            var checkDiscover = new Regex("^(6011[0-9]{12})$");
            ...
            // mc
            var checkMC = new Regex("^((?:51|55)[0-9]{14})$");
        ...
        internal static bool SaveBinaryImage(string stringImage, string filename)
        {
            ...
            try
            {
                using (var ms = new MemoryStream(Convert.FromBase64String(stringImage)))
                {
        ...
        public static bool create_mini_images(string url)
        {
            ...
            {
                Image png = Image.FromFile(full_url);
                var d = new DirectoryInfo(HttpContext.Current.Server.MapPath(path + "/_resize/"));
                if (!d.Exists)
                {
                    d.Create();
                }
        ...
        {
            return ConfigurationManager.AppSettings["client_site"] ?? string.Empty;
        }

        public static string GetAdminSite()
        {
            var rqUrl = HttpContext.Current.Request.Url;
            return $"{rqUrl.Scheme}://{rqUrl.Authority}";
        }
    }
