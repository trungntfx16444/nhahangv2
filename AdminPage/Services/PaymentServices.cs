...
namespace AdminPage.Services
{
    public class PaymentServices : ServicesBase
    {
        ...
        public string Callback(out string errMsg)
        {
            errMsg = string.Empty;
            webconfiguration info = UserContent.GetWebInfomation(true);
            ...
            if (checkSignature)
            {
                ...
                {
                    errMsg = VNPAY_MessageResp(vnp_ResponseCode, false);
                }
                //
            ...
            errMsg = "Có lỗi xảy ra trong quá trình xử lý";
            return orderId;
        }

        public string NotiPayment(ControllerContext context, order model)
        {
            webconfiguration info = UserContent.GetWebInfomation(true);
            string viewPath = "sendmail/NotiPayment";
            string body = AppFunc.RenderViewToString(context, viewPath, model, true);
            return SendEmail.Send(info.ContactEmail, $"Đơn hàng (#{model.Id}) đã được thanh toán ", body, string.Empty, string.Empty, null);
        }
