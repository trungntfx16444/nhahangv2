...
namespace AdminPage.Controllers
{
    ...
    [Authorize]
    public class FeatureController : UploadController
    {
        // GET: Admin/Feature
        ...
        public ActionResult Index(string lang, string code = "trangchu")
        {
            using (var db = new DBLangCustom())
            {
                db.setLanguage(lang);
                ViewBag.Category = db.categories.OrderBy(x => x.Order).ToList();
                var listSF = db.sectionfeatures.Where(x => x.Code.ToLower().StartsWith(code) && x.Hidden != true).OrderBy(x => x.Order).ToList();
                if (listSF == null || listSF.Count == 0)
                {
                    return Redirect("/feature?code=trangchu");
                }
                return View(listSF);
            }
        }
        ...
        public JsonResult GetFeatureByCode(string code)
        {
            try
            {
                ...
                var images = _db.uploadmorefiles.Where(u => u.TableId == result.Code && u.TableName == "sectionfeature").ToList();
                var images_partial = CommonFunc.RenderRazorViewToString("_UploadImages", images, this);
                return Json(new object[] { true, result, images_partial }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        /// <returns>json.</returns>
        public JsonResult Save(sectionfeature data, List<string> picmore, string lang)
        {
            try
            {
                if (!access.ContainsKey("update_features"))
                ...
                feature.LangCode = lang ?? SiteLang.GetDefault().Code;
                _db.Entry(feature).State = System.Data.Entity.EntityState.Modified;
                _db.SaveChanges();
                UploadMoreFiles(feature.Code, "sectionfeature", picmore?.ToArray());
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(data.Id))
                {
                    ...
                    var sfDetail = new sectionfeaturedetail
                    {
                        ...
                        ExternalHTML = Request.Unvalidated["ExternalHTML"],
                    };
                ...
                else
                {
                    ...
                    sfDetail.ExternalHTML = Request.Unvalidated["ExternalHTML"];
                    ...
                    //    sfDetail.URL = $"/{SEO_Product.url}?order={contents[0]}";
                    ...
                    //    sfDetail.Description = string.Empty;
                    //}
                    var url_product = Request["url_product"];
                    if (!string.IsNullOrEmpty(url_product))
                    {
                        sfDetail.URL = url_product;
                        sfDetail.Description = string.Empty;
                    }
                    else
                    if (string.IsNullOrEmpty(sfDetail.URL))
                    {
                        sfDetail.URL = "#";
                    }
        ...
        public void UpdateOrder(AdminEntities db, IQueryable<sectionfeaturedetail> all_sfDetail, string id, int? order)
        {
