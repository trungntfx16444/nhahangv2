namespace AdminPage.Controllers
{
    ...
    public class UploadController : ExpiredCheckController
    {
        ...
        /// <summary>
        /// UploadmoreFile dùng cho upload bằng ckfinder.
        /// </summary>
        /// <param name="filestotal"></param>
        /// <param name="strPath"></param>
        /// <param name="prefix"></param>
        /// <returns></returns>
        protected void UploadMoreFiles(string TableId, string TableName, string[] ListFilePaths)
        {
            AdminEntities _db = new();
            var more_picture = _db.uploadmorefiles.Where(x => x.TableId == TableId && x.TableName == TableName).ToList();
            if (more_picture.Any())
            {
            ...
            {
                foreach (var item in ListFilePaths)
                {
