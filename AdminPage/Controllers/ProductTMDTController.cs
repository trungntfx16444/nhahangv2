namespace AdminPage.Controllers
{
    ...
    [Authorize]
    public class ProductTMDTController : UploadController
    {
        // GET: Admin/ProductTMDT
        ...
        public JsonResult Load_table(DataTable_request data, string category, string search, string stock_status)
        {
            ...
            if (!string.IsNullOrEmpty(category))
            {
                ...
                orderQuery = orderQuery.Where(x => listCate.Contains(x.CategoryId));
            }
        ...
        public ActionResult Save(string id, string lang)
        {
            try
            {
                ...
                var listOptPrice = _db.productoptionprices.Where(x => x.ProductId == pro.ReId && x.IsActive == true).ToList();
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(p.ReId))
                {
                    foreach (var item in listPrice)
                    {
                        ...
                        _db.productoptionprices.Add(item);
                    }
                }
                else
                {
                    var listPrice_inDB = _db.productoptionprices.Where(x => x.ProductId == product.ReId && x.IsActive == true).ToList();
