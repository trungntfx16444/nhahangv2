namespace AdminPage.Controllers
{
    ...
    public class PaymentController : ExpiredCheckController
    {
        ...
        public JsonResult PaymentList(DataTable_request data, DateTime? From, DateTime To, string search)
        {
            ...
            var paymentList = db.vnp_paymentdata.Where(pay => orderId.Contains(pay.OrderId)).OrderBy(pay => pay.UpdatedAt).Join(db.orders, p=>p.OrderId, o=>o.Id, (p,o)=> new { p, o }).ToList().Select(item=>(item.p, item.o)).ToList();
