    ...
    public class MenuMgrController : ExpiredCheckController
    {
        ...
        public ActionResult Savemenu(menu data)
        {
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (string.IsNullOrEmpty(data.Id))
                    {
                        ...
                        var menu = new menu
                        {
                            ...
                            URL = data.URL,
                            Order = data.Order >0 ? data.Order: ((_db.menus?.Max(x => x.Order) ?? 0) + 1),
                ...
                return Redirect("/MenuMgr?langcode=" + data.LangCode);
            }
        }
            ...
            using (var trans = _db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    menu menu_parentLv2 = null;
                        ...
                        {
                            var menu = menus.FirstOrDefault(x => x.Id == list_id[i].Split('|')[1]);
                            if (menu == null)
                            {
