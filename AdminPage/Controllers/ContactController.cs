namespace AdminPage.Controllers
{
    ...
    public class ContactController : UploadController
    {
        ...
        public ActionResult Index(string lang)
        {
            ...
            var web_config = _db.webconfigurations.FirstOrDefault(x => x.LangCode == lang) ?? new webconfiguration();
            ...
            return View(web_config);
        }

        public JsonResult ActiveSale(bool? active)
        {
            try
            {
                foreach (var item in _db.webconfigurations)
                {
                    item.ActiveSales = active ?? false;
            ...
            {
                return Json(new object[] { false, $"Lỗi: {ex.Message}" });
            }
        }
        ...
        public JsonResult UpdateMetaSEO(SiteSEO.SEO_Meta dataSEO)
        {
            try
            {
                ...
                var web_config = _db.webconfigurations.FirstOrDefault(x => x.LangCode == langeCode);
                if (web_config == null)
                {
                ...
                _db.Entry(web_config).State = System.Data.Entity.EntityState.Modified;
                _db.SaveChanges();

                #region UPDATE URL MENU - LOC INACTIVE 06/04/2022
                //menu _menu = null;
                //if (dataSEO.code == SiteSEO.code_SEO.SanPham)
                //{
                //    _menu = _db.menus.FirstOrDefault(x => x.Id == "2" && x.LangCode == langeCode); // 2 => id menu san pham
                //}
                //else if (dataSEO.code == SiteSEO.code_SEO.TinTuc)
                //{
                //    _menu = _db.menus.FirstOrDefault(x => x.Id == "3" && x.LangCode == langeCode); // 2 => id menu tin tuc
                //}

                //if (_menu != null)
                //{
                //    _menu.URL = dataSEO.url;
                //    _db.Entry(_menu).State = System.Data.Entity.EntityState.Modified;
                //    _db.SaveChanges();
                //}
                #endregion
