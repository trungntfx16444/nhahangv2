namespace PKWebShop.Services
{
    ...
    {
        public List<expense> LoadExpense(DataTable_request data, DateTime? From, DateTime To, string search, string s_vendor, string s_status, out int[] dt, out string err_msg)
        {
            var listExpense = new List<expense>();
            err_msg = string.Empty;
            try
            {
                var recordsTotal = DB.expense.Count();
                IQueryable<expense> expenseQuery;

                if (!string.IsNullOrEmpty(search))
                {
                    var sqlCommand = CommonFunc.SearchCommand("expenses", search, "Id", "ImportTicket_Id");
                    expenseQuery = DB.expense.SqlQuery(sqlCommand).Where(x => (string.IsNullOrEmpty(s_vendor) || (s_vendor == "-1" && string.IsNullOrEmpty(x.Vendor_Id)) || x.Vendor_Id == s_vendor) && (string.IsNullOrEmpty(s_status) || x.Status == s_status) && x.CreatedAt >= From && x.CreatedAt <= To).AsQueryable();
                }
                else
                {
                    expenseQuery = DB.expense.Where(x => (string.IsNullOrEmpty(s_vendor) || (s_vendor == "-1" && string.IsNullOrEmpty(x.Vendor_Id)) || x.Vendor_Id == s_vendor) && (string.IsNullOrEmpty(s_status) || x.Status == s_status) && x.CreatedAt >= From && x.CreatedAt <= To);
                }
        ...
        public string SaveExpense(expense data, string totalStr, string expenseName, string expensePrice)
        {
            ...
            if (!string.IsNullOrEmpty(data.Vendor_Id))
            {
                ...
                {
                    importTicket = DB.import_tickets.FirstOrDefault(x => x.Code == data.ImportTicket_Id && x.Vendor_Id == data.Vendor_Id);
            ...
            {
                importTicket = DB.import_tickets.FirstOrDefault(x => x.Code == data.ImportTicket_Id);
                if (importTicket == null)
                {
                    throw new Exception("Mã phiếu nhập không đúng");
                }
            ...
            {
                var exp = DB.expense.FirstOrDefault(x => x.Id == data.Id);
                if (exp == null)
                {
            ...
            {
                if (DB.expense.Where(x => x.ImportTicket_Id == importTicket.Code && x.Status == "1").Sum(x => x.Total) >= importTicket.Total)
                {
