namespace PKWebShop.Areas.Admin.Controllers
{
    ...
    public class InventoryController : ExpiredCheckController
    {
        ...
        {
            var recordsTotal = _db.import_tickets.Count();
            var tk = _db.import_tickets.Select(i => new
            {
                import_ticket = i,
        ...
        public JsonResult ImportTicketGet(string Id)
        {
            ...
            {
                var db = new DBLangCustom();
                var it = _db.import_tickets.Find(Id);
        ...
        public JsonResult ImportTicketQuickSave(ICollection<submit_product> submit_products)
        {
            try
            {
                ...
                submit_products.GroupBy(s => s.Vendor_Id).ForEach(s =>
                {
                    ...
                    if (!int.TryParse(_db.import_tickets.Select(i => i.Code).Where(c => c.StartsWith(monthCode)).Max()?.Substring(4), out int max))
                    {
                        max = 0;
                    }

                    it.Code = monthCode + (max + 1).ToString().PadLeft(4, '0');
                    _db.import_tickets.Add(it);
                });
        ...
        public JsonResult ImportTicketSave(import_ticket it, ICollection<submit_product> submit_product, List<string> OtherPriceName, List<decimal?> OtherPriceAmount, TimeSpan? OrderTime, TimeSpan? ReceivedTime, ImportTicket_PaymentStatus? PayStatus, ImportTicket_ImportStatus ImportStatus)
        {
            try
            {
                ...
                if (addnew)
                {
                    ...
                    if (!int.TryParse(_db.import_tickets.Select(i => i.Code).Where(c => c.StartsWith(monthCode)).Max()?.GetLast(4), out int max))
                    {
                        max = 0;
                    }
                ...
                {
                    tk1 = _db.import_tickets.Find(it.Id);
                    if (tk1 == null)
                    {
                ...
                _db.import_tickets.AddOrUpdate(it);
                _db.SaveChanges();
        ...
        public JsonResult VendorSave(vendor vendor)
        {
            try
            {
                var addnew = string.IsNullOrEmpty(vendor.Id);
                if (addnew)
                {
                ...
                {
                    var vendor1 = _db.vendors.Find(vendor.Id);
                    if (vendor1 == null)
                    {
                ...
                _db.SaveChanges();
                return Json(new object[] { true, addnew ? "Tạo nhà cung cấp mới hoàn tất" : "cập nhật nhà cung cấp hoàn tất!" });
            }
            catch (Exception ex)
            {
                return Json(new object[] { false, ex.Message, ex.ToString() });
