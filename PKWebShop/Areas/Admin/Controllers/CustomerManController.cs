namespace PKWebShop.Areas.Admin.Controllers
{
    ...
    public class CustomerManController : ExpiredCheckController
    {
        ...
        public JsonResult SaveCustomer(customer data)
        {
            try
            {
                if (string.IsNullOrEmpty(data.FullName) || string.IsNullOrEmpty(data.Phone))
                {
                    throw new Exception("Vui lòng điền đầy đủ thông tin");
                }
                
                var cusExist = db.customers.Find(data.Id);
                if (cusExist == null)
                {
                    throw new Exception("Khách hàng không tồn tại");
                }
                else if (db.customers.Any(x => x.Phone == data.Phone && x.Id != data.Id))
                {
                ...
                db.Entry(cusExist).State = System.Data.Entity.EntityState.Modified;
                db.SaveChanges();
        ...
        public JsonResult ResetPassword(string Id)
        {
            try
            {
                ...
                customer.Password = DateTime.Now.ToString("mmssffff");
                db.Entry(customer).State = System.Data.Entity.EntityState.Modified;
                db.SaveChanges();

                string msg_success = $"Mật khẩu mới: <b>{customer.Password}</b>";
