...
namespace PKWebShop.Areas.Admin.Controllers
{
    ...
    [Authorize]
    public class OrderTMDTController : ExpiredCheckController
    {
        // GET: Admin/OrderTMDT
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.new_orders = db.orders?.Count(x => x.Status == "0");
            ViewBag.total_orders = db.orders?.Count();
            ViewBag.new_sales = db.orders?.Where(x => x.Status == "0").Select(x=> ((double?)x.GrandTotal)).Sum() ?? 0;
            ViewBag.total_sales = db.orders?.Sum(x => x.GrandTotal ?? 0) ?? 0;
            return View();
        }
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        foreach (var item in lstOrderDetail_Database)
                        {
                            ...
                            {
                                qtys.Add((item.ProductId, item.Quantity ?? 0, 0));
                            ...
                            else if (listProductItem.Any(x => x.Id == item.Id))
                            {
                                ...
                                qtys.Add((item.ProductId, item.Quantity ?? 0, proItem.Quantity ?? 0));
                        ...
                        foreach (var item in listProductItem)
                        {
                            if (!lstOrderDetail_Database.Any(x => x.Id == item.Id))
                            {
                                qtys.Add((item.ProductId, 0, item.Quantity ?? 0));
                        ...
                        qtys.ForEach(qty =>
                        {
                            products.Where(p => p.ReId == qty.id).ToList().ForEach(p =>
                            {
                                p.Quantity ??= 0;
                                p.Quantity -= qty.now - qty.old;
                                db.Entry(p).State = System.Data.Entity.EntityState.Modified;
                            });
                        });

                        orderExist.SubTotal = listProductItem.Sum(x => x.Quantity * x.Price);
        ...
        public ActionResult LoadProduct(string cateReId)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(cateReId))
                {
                    ...
                    listProduct = listProduct.Where(x => listCate.Contains(x.CategoryId));
                }
        ...
        public ActionResult GetProductDetail(string proReId, string orderDetail)
        {
            try
            {
                ...
                var arrObj = new object[]
                {
                    ...
                    propExist,
                    orderDetail,
                    db.ProductOptionPrice.Where(x => x.ProductId == proReId && x.IsActive == true && x.LangCode == languageDefault.Code).ToList(),
                };
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(_orderDetail))
                {
                    ...
                    if (proItem_exist == null)
                    {
                        ...
                        {
                            proItem.ProductName += $" ({db.ProductOptionPrice.FirstOrDefault(x => x.ReId == price_optReId && x.LangCode == language_default.Code)?.LabelName})";
                        }
                ...
                else
                {
                    ...
                    if (proItem_exist == null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(price_optReId))
                        {
                            proItemUpdate.ProductName += $" ({db.ProductOptionPrice.FirstOrDefault(x => x.ReId == price_optReId && x.LangCode == language_default.Code)?.LabelName})";
                        }
                    }
                    else
                    {
