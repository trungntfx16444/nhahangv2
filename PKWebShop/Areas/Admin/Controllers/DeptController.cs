namespace PKWebShop.Areas.Admin.Controllers
{
    ...
    using ExpenseService = PKWebShop.Services.ExpenseService;
    ...
    public class DeptController : ExpiredCheckController
    {
        ...
        public ActionResult ReceiptSave(receipts item)
        {
            try
            {
                _receipts.Save(item);
                return Json(new object[] { true, string.IsNullOrEmpty(item.Id) ? "Đã tạo phiếu thu hòa tất" : "Đã cập nhật phiếu thu" });
            }
            catch (Exception e)
            {
        ...
        public ActionResult Expense(string search = "")
        {
            ...
            // report
            var importTicketTotal = db.import_tickets.Where(x => x.ImportStatus == "Imported").Sum(x => x.Total) ?? 0;
            var expenseTotal = db.expense.Where(x => x.Status == "1").Sum(x => x.Total) ?? 0;
        ...
        public JsonResult GetExpense(string id)
        {
            ...
            {
                WebShopEntities db = new WebShopEntities();
                var expense = db.expense.FirstOrDefault(x => x.Id == id);
                if (expense == null)
                {
        ...
        public JsonResult SaveExpense(expense data)
        {
            try
            {
        ...
        public ActionResult _ExpenseDetail(string Id)
        {
            ...
            {
                WebShopEntities db = new WebShopEntities();
                var exp = db.expense.FirstOrDefault(x => x.Id == Id);
                if (exp == null)
                {
